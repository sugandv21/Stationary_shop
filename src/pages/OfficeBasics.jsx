...
import addedToCartImg from "../assets/images/added_to_cart1.png"; 
...
const OfficeBasics = () => {
   useEffect(() => {
          document.title = "Supplyhive | Office Basics"
        }, []); 
  const [selectedPrices, setSelectedPrices] = useState([priceFilters[0].label]);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [addedProduct, setAddedProduct] = useState(null);
  const [selectedCategory, setSelectedCategory] = useState("Office Basics");
  ...
  return (
    <div className="min-h-screen ">
        ...
        <p className="text-center mt-10 text-lg md:text-2xl font-semibold text">
              ...
              {categories.map((c, i) => (
                <li key={i} className="hover:underline">
                  <Link
                    ...
                    onClick={() => handleCategoryClick(c)}
                    className={`block ${
                      selectedCategory === c ? "text-black font-bold" : "text-white"
                    }`}
                  >
                    {c}
                  </Link>
            ...
            {filteredProducts.map((p) => (
              ...
              >
                ...
                <div className="p-4 bg-[#7aabf5] text-center">
                  ...
                  {/* Button with Added-to-Cart Image */}
                  <div className="relative mt-3">
                    {addedProduct === p.id && (
                      <img
                        src={addedToCartImg}
                        alt="Added to Cart"
                        className="absolute -top-24 left-1/2 transform -translate-x-1/2 w-32 h-32 object-contain transition-all"
                      />
                    )}
